class Api::V1::ListingType::ListingsController < ApplicationController
    ...
    def set_listings
        if params[:swlat]
            ...
            @listings = Listing.order(created_at: :desc).near([params[:latitude], params[:longitude]], 14).page(params[:page]).per(20)
            @total_pages = Listing.near([params[:latitude], params[:longitude]], 14).page(1).per(20).total_pages
        else 
